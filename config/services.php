...
return static function (ContainerConfigurator $containerConfigurator): void {
    ...
    $services->instanceof(HealthCheck::class)->tag('app.health_check');
    ...
    $services->set(ExceptionListener::class)->arg(
        '$environment',
        '%kernel.environment%',
    ...
    $services->set(JsonBodyResolver::class)->tag('controller.argument_value_resolver', [
        'priority' => -50,
    ]);

    $services->set(EnableSqliteWalMiddleware::class)->tag('doctrine.middleware');

    $services->set(RegisterTotpSecretTypeMiddleware::class)->tag('doctrine.middleware');

    $services->set(HealthCheckRunner::class)->arg('$checks', tagged_iterator('app.health_check'));

    $services->set(HealthEvaluator::class);

    $services->set(ConnectionInputMapper::class);

    $services->set(CollectorRegistry::class)->args([
        inline_service(InMemory::class),
        false,
    ]);
    ...
    $services->set(OidcConfig::class)->factory([service(OidcConfigFactory::class), 'create']);
};
