...
final readonly class TextFormatRenderer
{
    ...
    {
        $this->registry
            ->getOrRegisterGauge(self::NAMESPACE, "build_info", "Build information of the NetPulse exporter.", ["version"])
    ...
    {
        $gauges = [
            ["download_bits_per_second", "Last measured download throughput in bits per second.", static fn(LatestMeasurementRow $r): ?float => $r->downloadBits === null ? null : (float)$r->downloadBits],
            ["upload_bits_per_second", "Last measured upload throughput in bits per second.", static fn(LatestMeasurementRow $r): ?float => $r->uploadBits === null ? null : (float)$r->uploadBits],
            ["ping_seconds", "Last measured idle latency in seconds.", static fn(LatestMeasurementRow $r): ?float => $r->pingSeconds],
            ["jitter_seconds", "Last measured idle jitter in seconds.", static fn(LatestMeasurementRow $r): ?float => $r->jitterSeconds],
            ["packet_loss_ratio", "Last measured packet loss as a ratio between 0 and 1.", static fn(LatestMeasurementRow $r): ?float => $r->packetLossRatio],
            ["download_latency_iqm_seconds", "Last measured loaded download latency (IQM) in seconds.", static fn(LatestMeasurementRow $r): ?float => $r->downloadLatencyIqmSeconds],
            ["upload_latency_iqm_seconds", "Last measured loaded upload latency (IQM) in seconds.", static fn(LatestMeasurementRow $r): ?float => $r->uploadLatencyIqmSeconds],
            ["data_used_bytes", "Bytes transferred during the last measurement.", static fn(LatestMeasurementRow $r): ?float => $r->dataUsedBytes === null ? null : (float)$r->dataUsedBytes],
        ];
        ...
        foreach ($gauges as [$name, $help, $value]) {
            $gauge = $this->registry->getOrRegisterGauge(
                ...
                $name,
                $help,
                ["probe", "connection", "server_name", "server_id", "isp"],
            );
            ...
            foreach ($latest as $row) {
                ...
                $gauge->set($resolved, [$row->probeName, $row->connectionName, $row->serverName, $row->serverId, $row->isp]);
            }
        }
    }
    ...
    private function populateConnectionHealthy(LatestMeasurementRowCollection $latest): void
    {
        ...
        foreach ($latest as $row) {
            ...
            $gauge->set(
                $row->healthy ? 1 : 0,
                [$row->probeName, $row->connectionName, $row->site, $row->serverName, $row->serverId, $row->isp],
            );
        }
    }
    ...
    {
        $gauges = [
            ["connection_expected_download_bits_per_second", "Configured expected download speed in bits per second.", static fn(ExpectedRow $r): int => $r->expectedDownloadBits],
            ["connection_expected_upload_bits_per_second", "Configured expected upload speed in bits per second.", static fn(ExpectedRow $r): int => $r->expectedUploadBits],
        ];
    ...
    {
        $this->registry
            ->getOrRegisterCounter(self::NAMESPACE, "remote_write_failures_total", "Total number of remote write push failures.", [])
