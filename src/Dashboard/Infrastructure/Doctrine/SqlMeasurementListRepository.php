...
final readonly class SqlMeasurementListRepository implements MeasurementListRepository
{
    ...
    private function applyFilter(QueryBuilder $queryBuilder, MeasurementFilter $filter): void
    {
        ...
        if ($filter->connection !== null) {
            $queryBuilder
                ->andWhere("measurement.connectionId = :connection")
                ->setParameter("connection", $filter->connection, "connection_id");
        }

        if ($filter->serverId !== null) {
            $queryBuilder
                ->andWhere("measurement.serverId = :serverId")
                ->setParameter("serverId", $filter->serverId);
        }

        if ($filter->status !== null) {
            $queryBuilder
                ->andWhere("measurement.status = :status")
                ->setParameter("status", $filter->status->value);
        }

        if ($filter->healthy !== null) {
            $queryBuilder
                ->andWhere("measurement.healthy = :healthy")
                ->setParameter("healthy", $filter->healthy, Types::BOOLEAN);
        }

        if ($filter->scheduled !== null) {
            $queryBuilder
                ->andWhere("measurement.scheduled = :scheduled")
                ->setParameter("scheduled", $filter->scheduled, Types::BOOLEAN);
        }
    }
}
