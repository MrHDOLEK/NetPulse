...
final class DoctrineNotificationSendCounterTest extends KernelTestCase
{
    ...
    private function total(string $kind, string $channel, string $status): int
    {
        $value = $this->connection->fetchOne(
            "SELECT total FROM notification_send_counts WHERE kind = :kind AND channel = :channel AND status = :status",
    ...
    {
        $value = $this->connection->fetchOne(
            "SELECT COUNT(*) FROM notification_send_counts WHERE kind = :kind AND channel = :channel AND status = :status",
            ["kind" => $kind, "channel" => $channel, "status" => $status],
        );
