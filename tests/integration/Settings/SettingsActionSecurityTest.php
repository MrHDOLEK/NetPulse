...
final class SettingsActionSecurityTest extends KernelTestCase
{
    ...
    {
        $value = $this->db->fetchOne('SELECT value FROM app_settings WHERE setting_key = ?', [$key->value]);
    ...
    {
        $this->session->set(SessionTokenStorage::SESSION_NAMESPACE . '/' . self::CSRF_LOGIN_ID, self::CSRF_LOGIN_RAW);
