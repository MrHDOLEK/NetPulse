...
final class DashboardPageTest extends KernelTestCase
{
    ...
    public function testAuthenticatedDashboardRendersRealReadModelData(): void
    {
        ...
        self::assertStringContainsString('name="range"', $body);

        foreach (["24h", "7d", "30d", "90d"] as $window) {
            self::assertStringContainsString(sprintf('value="%s"', $window), $body);
        }

        self::assertStringContainsString(self::CONN_NAME, $body);
        self::assertStringNotContainsString("Nothing to see yet", $body);

        self::assertStringContainsString('id="main-chart"', $body);

        foreach (["Download", "Upload", "Ping", "Packet loss"] as $label) {
            self::assertStringContainsString($label, $body);
        }

        self::assertStringContainsString("Uptime", $body);
        self::assertStringContainsString("Connection health", $body);
        self::assertStringContainsString("Recent results", $body);
        self::assertStringContainsString("GET /metrics → 200", $body);
        self::assertStringContainsString("Latest test", $body);

        self::assertStringContainsString('id="dashboard-bootstrap"', $body);
    }
