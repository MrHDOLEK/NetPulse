...
final class HistoryApiTest extends KernelTestCase
{
    ...
    public function testReturnsFullMeasurementDetailJson(): void
    {
        ...
        foreach ([
            ...
            'pingHighSeconds',
            'jitterSeconds',
            'downloadLatencyIqmSeconds',
            'uploadLatencyIqmSeconds',
            'packetLossRatio',
            'dataUsedDownload',
            'dataUsedUpload',
        ] as $field) {
        ...
        foreach ([
            ...
            'pingHighLabel',
            'jitterLabel',
            'downloadLatencyIqmLabel',
            'uploadLatencyIqmLabel',
            'lossLabel',
        ] as $field) {
    ...
    {
        $now = new DateTimeImmutable('now', new DateTimeZone('UTC'))->getTimestamp();
        $startedAt = new DateTimeImmutable('@' . ($now - 3700))
            ->setTimezone(new DateTimeZone('UTC'))
            ->format('Y-m-d H:i:s');
        $completedAt = new DateTimeImmutable('@' . ($now - 3600))
            ->setTimezone(new DateTimeZone('UTC'))
            ->format('Y-m-d H:i:s');
