...
final class OidcAuthenticatorTest extends TestCase
{
    ...
    public function testVerifiedEmailWithExistingUserYieldsPassportForThatEmail(): void
    {
        ...
        $authenticator = $this->authenticator(
            new StubOidcProvider(new OidcIdentity("sub-1", self::ADMIN_EMAIL, true, "Admin")),
        );
    ...
    public function testUnverifiedEmailIsDenied(): void
    {
        $this->seedAdmin();
        $authenticator = $this->authenticator(
            new StubOidcProvider(new OidcIdentity("sub-1", self::ADMIN_EMAIL, false, "Admin")),
        );
    ...
    {
        $authenticator = $this->authenticator(
            new StubOidcProvider(new OidcIdentity("sub-9", "stranger@example.com", true, "Stranger")),
        );
