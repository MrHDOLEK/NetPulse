...
final class TextFormatRendererTest extends TestCase
{
    ...
    public static function provideExpectedLines(): iterable
    {
        ...
        yield 'connection_info sample' => [
            'netpulse_connection_info{probe="home",connection="wan1",site="home-lab",isp="Acme ISP"} 1',
        ];
        ...
        yield 'download gauge' => [
            'netpulse_download_bits_per_second{probe="home",connection="wan1",server_name="Acme Speedtest",server_id="12345",isp="Acme ISP"} 950000000',
        ];
        yield 'ping gauge float' => [
            'netpulse_ping_seconds{probe="home",connection="wan1",server_name="Acme Speedtest",server_id="12345",isp="Acme ISP"} 0.012',
        ];
        yield 'packet loss zero' => [
            'netpulse_packet_loss_ratio{probe="home",connection="wan1",server_name="Acme Speedtest",server_id="12345",isp="Acme ISP"} 0',
        ];
        yield 'last result timestamp' => [
            'netpulse_last_result_timestamp_seconds{probe="home",connection="wan1"} ' . (self::NOW - 100),
        ];

        yield 'expected download' => [
        ...
        yield 'connection_healthy false' => [
            'netpulse_connection_healthy{probe="home",connection="wan2",site="home-lab",server_name="Other Server",server_id="67890",isp="Acme ISP"} 0',
        ];
        ...
        yield 'notifications_sent alert webhook' => [
            'netpulse_notifications_sent_total{kind="alert",channel="webhook",status="sent"} 1',
        ];
        yield 'notifications_sent recovery webhook' => [
            'netpulse_notifications_sent_total{kind="recovery",channel="webhook",status="sent"} 1',
        ];
        yield 'notifications_sent digest webhook' => [
            'netpulse_notifications_sent_total{kind="digest",channel="webhook",status="sent"} 1',
        ];
        yield 'notifications_sent alert email failed' => [
            'netpulse_notifications_sent_total{kind="alert",channel="email",status="failed"} 2',
        ];
        yield 'notifications_sent digest webhook skipped' => [
            'netpulse_notifications_sent_total{kind="digest",channel="webhook",status="skipped"} 1',
        ];
    }
    ...
    public function testSkipsNullGaugeValuesFromFailedMeasurements(): void
    {
        ...
        self::assertStringNotContainsString(
            'netpulse_download_bits_per_second{probe="home",connection="wan3"',
            $output,
        );
