...
final class Version20260606123154 extends AbstractMigration
{
    ...
    {
        $this->addSql("CREATE TEMPORARY TABLE __temp__connections AS SELECT probe_id, name, isp, color, labels, server_pool, enabled, id, expected_download_bits, expected_upload_bits, schedule FROM connections");
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql("INSERT INTO connections (probe_id, name, isp, color, labels, server_pool, enabled, id, expected_download_bits, expected_upload_bits, schedule) SELECT probe_id, name, isp, color, labels, server_pool, enabled, id, expected_download_bits, expected_upload_bits, schedule FROM __temp__connections");
        $this->addSql("DROP TABLE __temp__connections");
        $this->addSql("CREATE INDEX idx_connections_probe_id ON connections (probe_id)");
    }
}
